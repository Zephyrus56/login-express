...
module.exports = {
    ...
    createAccount: (req, res) => {
        ...
        const verify = payloadCheck(req.body, payload, ['name', 'email', 'password', 'phone'])
        if(!verify.status) return ERROR(res, 501, false, verify.message)
    ...
    checkUser: (req, res) => {
        checkUserId({ id: req.decoded.user.id}, (error, result) => {
            if(error) return ERROR(res, 500, false, error)
    ...
    updateUser: (req, res) => {
        ...
        updateUser({ 
            name: req.body.name, email: req.body.email, phone: req.body.phone, address: req.body.address, gender: req.body.gender, id: req.decoded.user.id,
        }, (error, result) => {
            if(error) return ERROR(res, 500, false, error)
    ...
    deleteUser: (req, res) => {
        deleteUser({ id: req.decoded.user.id }, (error, result) => {
            if(error) return ERROR(res, 500, false, error)
